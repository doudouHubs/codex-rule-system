---
name: rule-system
description: Use when the user asks about the Rule System plugin, session-rule boundaries, project-shared rule libraries, how rule-add/project-rule-* skills relate, or wants global prompt rule-system guidance moved into plugin-managed instructions.
---
...
它不直接写规则；它负责解释规则系统边界、路由关系和全局提示词瘦身原则。具体读写动作交给 `$rule-*` 与 `$project-rule-*`。
...
- 项目规则库不会自动注入会话；必须通过 `$project-rule-pick` 显式拾取后，才进入当前会话规则。
- `$project-rule-pick` 是快照复制，不是动态引用；项目规则后续修改不会悄悄改变已拾取的会话规则。
...
- 用户要求新增项目级共享规则：使用 `$project-rule-add`。
- 用户要求查看、搜索项目规则库：使用 `$project-rule-list`。
- 用户要求修改项目共享规则：使用 `$project-rule-update`。
- 用户要求废弃或删除项目共享规则：使用 `$project-rule-delete`。
- 用户要求把项目共享规则带入当前会话背景：使用 `$project-rule-pick`。
...
```text
规则收集、会话规则 CRUD、项目规则库、规则拾取、规则展示和上下文规则捕获由 rule-system 插件接管；遇到相关意图时使用对应 $rule-* 或 $project-rule-* skill。
```
...
- project-rule CRUD 与 pick 的操作流程和输出格式
