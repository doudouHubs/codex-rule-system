...
`$rule-add` 是新增规则的唯一入口。v0.3 起本地落盘的规则内容只有项目级，写入 `<project_root>/.codex-rules/rules.db`。
...
4. `--scope project` 可传但不改变语义；`--scope session` 已退役，必须失败。
5. 批量新增只认英文分号 `;`，且 `title` 与 `content` 两侧数量必须一致；中文分号 `；` 永远是普通正文标点。
6. 新增成功只表示进入项目规则库；若希望当前会话立即采用，需要显式使用 `$rule-check` 勾选。
