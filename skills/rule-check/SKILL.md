...
- UI 表格里的 `状态` 和 `模块` 单元格是状态/模块的唯一编辑入口；点击后下拉切换，选择后立即更新当前行内存态，最终以“保存并选取”提交。
- UI 表格里的 `标题`、`标签`、`内容预览` 单元格支持单行直接编辑；标题和内容空输入会保留原值，标签允许清空。
- UI 不再提供右侧“编辑当前规则”面板；表格单元格是唯一编辑入口。
- 底部固定提供 `取消 Esc` 和 `保存并选取 Enter`，避免规则列表较多时按钮被挤出可见区域。
...
- `pick --ui` 的复选框表示当前会话是否采用该规则；单击或高亮某行只表示当前表格焦点行。
- 搜索过滤只改变可见行，不清空已勾选规则。
- 未勾选任何规则时点击确认，表示当前会话不采用任何确认后的规则；表格内联编辑仍会随“保存并选取”提交。
